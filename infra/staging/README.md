...
- GitHub Actions(`staging-dev.yml`)가 SSH로 서버에 접속해 `docker compose up -d`를 수행합니다.
...
- `infra/staging/secrets/ait_mtls_cert.pem`
- `infra/staging/secrets/ait_mtls_key.pem`

docker-compose가 위 파일을 컨테이너 `/run/secrets/`로 마운트합니다.

## 3) 런타임 환경변수
`infra/staging/docker-compose.yml`의 `api.environment`에서 아래 변수를 사용합니다.
